...
<template>
  <div class="min-h-screen bg-gray-50 flex flex-col items-center justify-center px-4 text-center">
    <div class="max-w-md">
      <div class="text-8xl mb-6">{{ is404 ? '🗺️' : '⚠️' }}</div>
      <h1 class="text-7xl font-black text-brand-600 mb-3">{{ error.statusCode }}</h1>
      <h2 class="text-2xl font-bold text-gray-900 mb-3">
        {{ is404 ? 'Pagina nu a fost găsită' : 'Ceva nu a funcționat' }}
      </h2>
      <p class="text-gray-500 mb-8 leading-relaxed">
        {{ is404
          ? 'Pagina pe care o cauți nu există. Poate a fost mutată sau URL-ul este greșit.'
