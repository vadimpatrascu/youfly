...
<template>
  <div>
    <div class="bg-gray-950 text-white py-12 px-4 text-center">
      <h1 class="text-3xl font-black mb-2">{{ t('privacy.title') }}</h1>
  ...
  <div class="max-w-3xl mx-auto px-4 py-12">
    ...
    <nav class="bg-gray-50 rounded-xl p-4 mb-6 text-xs">
      <ol class="flex flex-wrap gap-x-4 gap-y-1 text-gray-500">
        <li v-for="i in 6" :key="i"><a :href="`#privacy-s${i}-title`" class="hover:text-brand-600 transition-colors">{{ t(`privacy.s${i}Title`) }}</a></li>
      </ol>
    </nav>
